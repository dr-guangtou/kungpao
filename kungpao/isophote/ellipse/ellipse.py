...
from .constant import DEFAULT_EPS, \
    DEFAULT_STEP, \
    DEFAULT_SCLIP, \
    TOO_MANY_FLAGGED, \
    DEFAULT_CONVERGENCY, \
...
class Ellipse(object):
    ...
    This algorithm is designed to fit elliptical isophotes to galaxy images.
    Its main input is a 2-dimensional numpy array with the image.

    The output is a list of instances of class Isophote.
    See the documentation for this class for details.
    For convenience, this list is packaged in an instance of class
    ...
    pre-defined, fixed semi-major axis length. The algorithm starts from a
    first guess elliptical isophote defined by approximate values for the
    X and Y center coordinates, ellipticity and position angle. Using these
    values, the image is sampled along an elliptical path, producing a
    1-dimensional function that describes the dependency of intensity
    (pixel value) with angle (E).

    The function is stored as a set of 1-D numpy arrays. The harmonic content
    of this function is analyzed by least-squares fitting to the function:

    y  =  y0 + A1 * sin(E) + B1 * cos(E) + A2 * sin(2 * E) + B2 * cos (2 * E)

    Each one of the harmonic amplitudes A1, B1, A2, B2 is related to a specific
    ellipse geometric parameter, in the sense that it conveys information
    regarding how much the parameter's current value deviates from the "true"
    one. To compute this deviation, the image's local radial gradient has to be
    taken into account too. The algorithm picks up the largest amplitude among
    the four, estimates the local gradient and computes the corresponding
    increment in the associated ellipse parameter. That parameter is updated,
    and the image is resampled. This process is repeated until any one of the
    ...
    3 - more than a given fraction of the elliptical sample points have no
        ...
        boundaries or because they where flagged out from the fit by
        sigma-clipping.

    In any case, a minimum number of iterations is always performed.
    If iterations stop because of reasons 2 or 3 above, then those ellipse
    parameters that generated the lowest absolute values for harmonic
    amplitudes will be used. At this point, the image data sample coming from
    ...
    The corresponding amplitudes (A3, B3, A4, B4), divided by the semi-major
    axis length and local intensity gradient, measure the isophote's
    deviations from perfect ellipticity (these amplitudes, divided by
    semi-major axis and gradient, are the actual quantities stored in the
    output Isophote instance).
    ...
    axis length. These parameters, their errors, other associated parameters,
    ...
    are obtained from the errors in the coefficients of the 1st and 2nd
    simultaneous harmonic fit. 3rd and 4th harmonic amplitude errors are
    obtained in the same way, but only after the 1st and 2nd harmonics are
    subtracted from the raw data.

    See error analysis in Busko, I., 1996, Proceedings of the Fifth
    Astronomical Data Analysis Software and Systems Conference, Tucson, PASP
    ...
    incremented/decremented following a pre-defined rule. At each step, the
    starting, first guess ellipse parameters are taken from the previously
    fitted ellipse that has the closest semi-major axis length to the current
    one. On low surface brightness regions (those having large radii), the
    small values of the image radial gradient can induce large corrections and
    meaningless values for the ellipse parameters.
    The algorithm has the ability to stop increasing semi-major axis based on
    several criteria, including signal-to-noise ratio.
    ...
    stability against any non-elliptical structure such as stars, spiral arms,
    HII regions, defects, etc.
    ...
    close to the actual center for the fit to work.

    An "object centerer" function helps to verify that the selected position
    can be used as starting point. This function scans a 10 X 10 window
    centered either on the X,Y coordinates in the Geometry instance passed to
    the constructor of the Ellipse class, or, if any one of them, or both,
    are set to None, on the input image frame center. In case a successful
    acquisition takes place, the Geometry instance is modified in place to
    ...
    parameter.
    The centerer works by looking to where a quantity akin to a signal-to-noise
    ratio is maximized within the 10 X 10 window. The centerer can thus be shut
    off entirely by setting the threshold to a large value >> 1 (meaning, no
    location inside the search window will achieve that signal-to-noise ratio).

    A note of caution: the `ellipse` fitting algorithm was designed explicitly
    with a (elliptical) galaxy brightness distribution in mind. In particular,
    a well defined negative radial intensity gradient across the region being
    fitted is paramount for the achievement of stable solutions.

    Use of the algorithm in other types of images (e.g., planetary nebulae) may
    lead to inability to converge to any acceptable solution.
    ...
    def __init__(self,
        ...
        if geometry:
            ...
            self._geometry = Geometry(_x0, _y0, 8.,
                                      DEFAULT_EPS,
                                      (np.pi / 2))
    ...
    def fit_image(self,
        ...
        Fits multiple isophotes on the image array passed to the constructor.
        This method basically loops over each one of the values of semi-major
        axis length (sma) constructed from the input parameters, and fits one
        isophote at each sma, returning the entire set of isophotes in
        ...
        sma0 : float, default = None.
            ...
            This can't be neither the minimum or the maximum, but something in
            between. The algorithm can't start from the very center of the
            galaxy image because the modelling of elliptical isophotes on that
            region is poor and it will diverge very easily if not tied to othe
            previously fit isophotes. It can't start from the maximum value
            either because the maximum is not known beforehand, depending on
            signal-to-noise.
            The sma0 value should be selected such that the corresponding
            isophote has a good signal-to-noise ratio and a clearly defined
            geometry. If set to None (the default), one of two actions
            will be taken: in case an internal Geometry instance was passed to
            the Ellipse constructor, the `sma` in that instance will be used.
            If no Geometry instance was passed to the constructor, the default
        ...
        while True:
            ...
            minit_use = (2 * minit) if first_isophote else minit
            first_isophote = False

            isophote = self.fit_isophote(
                sma,
                step,
                conver,
                minit_use,
            ...
            if (isophote.stop_code < 0) or (isophote.stop_code ==
                                            TOO_MANY_FLAGGED):
                ...
                # If two consecutive isophotes failed to fit, shut off
                # iterative mode. Or, bail out and change to go inwards.
                if len(isophote_list) > 2:
                    if (isophote.stop_code == FAILED_FIT and
                        isophote_list[-2].stop_code == FAILED_FIT) \
                        or (isophote.stop_code ==
                            TOO_MANY_FLAGGED):
                        if maxsma and (maxsma > isophote.sma):
                            # If a maximum sma value was provided by user,
                            # and the current sma is smaller than maxsma,
                            # keep growing sma in non-iterative mode until
                            # reaching it.
                            noiter = True
                        else:
    ...
    def _iterative(self,
        ...
        if sma > 0.:
            ...
            # SH: This part needs to be checked.
    ...
    def _non_iterative(self,
        ...
        isophote = Isophote(sample, 0, True,
                            FIXED_ELLIPSE)
