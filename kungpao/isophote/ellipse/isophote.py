"""Classes for isophote and isophoteList."""
...
__all__ = ['CentralPixel', 'Isophote', 'IsophoteList', 'print_header']


def print_header(verbose=False):
    '''
    Print the header for output if verbose=True
    '''
    if verbose:
        print('#')
        print('# Semi-      Isophote         Ellipticity    Position     ' +
              'Grad.   Data  Flag Iter. Stop')
        print('# major        mean                           ' +
              'Angle        rel.                    code')
        print('# axis       intensity                                    ' +
              'error')
        print('#(pixel)                                     (degree)')
        print('#')
...
class Isophote:
    ...
    def __init__(self, sample, niter, valid, stop_code):
        ...
        The actual extracted sample at the given isophote (sampled intensities
        along the elliptical path on the image) is kept as an attribute of
        this class.

        The container concept helps in segregating information directly related
        to the sample, from information that more closely relates to the
        fitting process, such as status codes, errors for isophote parameters
        (as defined by the old STSDAS code), and the like.
        ...
        :param pix_stddev: float
            estimate of pixel standard deviation
            (rms * sqrt(average sector integration area))
        :param grad: float
        ...
        :param a3, b3, a4, b4: float
            ...
            ellipse.
            These values ar actually the raw harmonic amplitude divided by the
            local radial gradient and the semi-major axis length, so they can
            directly be compared with each other.
        ...
        iso_fluxes = self._compute_fluxes()
        self.tflux_e, self.tflux_c, self.npix_e, self.npix_c = iso_fluxes

        self._compute_errors()

        # deviations from a perfect ellipse
        self.a3, self.b3, self.a3_err, self.b3_err = self._compute_deviations(
            sample, 3)
        self.a4, self.b4, self.a4_err, self.b4_err = self._compute_deviations(
            sample, 4)
    ...
    def _compute_fluxes(self):
        '''
        Compute integrated flux inside ellipse, as well as inside circle
        defined by semi-major axis.

        Pixels in a square section enclosing circle are scanned;
        the distance of each pixel to the isophote center is compared both
        with the semi-major axis length and with the length of the ellipse
        radius vector, and integrals are updated if the pixel distance is
        smaller.
        '''
    ...
    def _compute_errors(self):
        ...
        try:
            ...
            # parameter errors result from direct projection of coefficient
            # errors.
            # These showed to be the error estimators that best convey the
            # errors measured in Monte Carlo experiments
            # (see reference in ellipse help page).
            ...
            self.ellip_err = abs(2. * errors[4] *
                                 (1. - eps) / self.sma / self.grad)
            if (abs(eps) > np.finfo(float).resolution):
                self.pa_err = abs(2. * errors[3] *
                                  (1. - eps) / self.sma / self.grad /
                                  (1. - (1. - eps)**2))
            else:
                self.pa_err = 0.
    ...
    def output(self, verbose=False):
        if verbose:
            if self.grad_r_error:
                s = "%7.2f   %9.2f (%5.2f) % 5.3f (%5.3f) %6.2f (%4.1f)  %5.3f  %4i  %4i %4i  %4i" % (
                    self.sample.geometry.sma, self.intens, self.int_err,
                    self.sample.geometry.eps, self.ellip_err,
                    self.sample.geometry.pa / np.pi * 180.,
                    self.pa_err / np.pi * 180., self.grad_r_error, self.ndata,
                    self.nflag, self.niter, self.stop_code)
            else:
                s = "%7.2f   %9.2f (%5.2f) % 5.3f (%5.3f) %6.2f (%4.1f)  None   %4i  %4i %4i  %4i" % (
                    self.sample.geometry.sma, self.intens, self.int_err,
                    self.sample.geometry.eps, self.ellip_err,
                    self.sample.geometry.pa / np.pi * 180.,
    ...
    def fix_geometry(self, isophote):
        ...
        an isophote with bad geometry, such as with eps>1 or another
        meaningless situation.

        This is not a problem in itself when fitting any given isophote,
...
class IsophoteList(Isophote, list):
    ...
    by numpy array attributes.

    These arrays reflect the values of the given attribute across the entire
    list of Isophote instances provided at constructor time.

    The class extends the 'list' built-in, thus providing basic list behavior
    such as slicing, appending, and support for '+' and '+=' operators.
    '''
